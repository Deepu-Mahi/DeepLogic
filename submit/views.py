...
def ask_ai_for_boilerplate(request):
    if request.method == "POST":
        if not request.user.is_authenticated:
            request.session['pending_ai_boilerplate'] = json.loads(request.body)
            return JsonResponse({"redirect": "/auth/login/?next=/ask_ai_boilerplate/"}, status=401)
...
def gemini_ai(request):
    if request.method == 'POST':
        if not request.user.is_authenticated:
            request.session['pending_ai_fix'] = json.loads(request.body)
            return JsonResponse({'redirect': '/auth/login/?next=/gemini-ai/'}, status=401)
...
def submit(request):
    ...
    pending = request.session.pop('pending_code', None)
    if pending and request.user.is_authenticated:
        output, errors = run_code(pending['language'], pending['code'], pending['input_data'])
        return render(request, "index.html", {
...
def problem_detail(request, problem_id):
    ...
    if request.method == 'POST':
        if not request.user.is_authenticated:
            request.session['pending_problem'] = {
        ...
        if not language or not code:
            ...
            if action == "run":
                ...
                for test_case in problem.test_cases.all():
                    actual_output, errors = run_code(language, code, test_case.input_data)
                    passed = (actual_output.strip() == test_case.expected_output.strip()) and (not errors)
                    test_results.append({
    ...
    pending = request.session.pop('pending_problem', None)
    if pending and pending.get('problem_id') == problem_id and request.user.is_authenticated:
        language = pending['language']
        code = pending['code']
        custom_input = pending.get('custom_input', '')
